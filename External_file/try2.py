...
class mythread_Demo(QtCore.QThread):
	...
	while line and self.win_draw.check_stopDemo==0:
	    if 'created_at' in line and '"lang":"en"' in line and ('korea' in line.lower() or 'kor' in line.lower()) :
...
class mythread(StreamListener, QtCore.QThread):
	...
	try:
		...
		if 'created_at' in data and '"lang":"en"' in data and ('' in data.lower() or '' in data.lower()) :
			t=int(calctime(initime))
		        #f.write(data)
		        #print(data)
    ...
    #while True:
    	...
    	twitter_stream.filter(track=['earthquake','quake', 'shake','flood'])

...
GPU command:
    THEANO_FLAGS=mode=FAST_RUN,validateice=gpu,floatX=float32 python lstm_rnns_aidr.py
...
#    	,featFile_validate   = "../data/good_vs_bad/CQA-QL-validateel.xml.multi.csv.feat"
	...
	print('LOADING DATA...')
	print('----------------------------------------------------------------')
	(X_train, y_train), (X_test, y_test), (X_validate, y_validate), (X_newinput, y_newinput), max_features, E, label_id = aidr.load_and_numberize_data(path=options.data_dir, nb_words=options.max_features, init_type=options.init_type, embfile=options.emb_file, validate_train_merge=0, map_labels_to_five_class=0)
...
#	print("Padding sequences....")
	...
	print(len(X_validate),   'validate   tweets')
	...
	if nb_classes == 2: # binary
		...
		y_validate_mod   = np_utils.to_categorical(y_validate,   nb_classes)
	...
	if nb_classes == 2: # binary
		...
		model.fit(X_train, y_train, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				validation_data=(X_validate, y_validate), verbose=2, callbacks=[earlystopper, checkpointer])#-quan bo show_accuracy --> accuracy 
...
#		print('Test accuracy:', acc)
		...
		model.fit(X_train, y_train_mod, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				validation_data=(X_validate, y_validate_mod), show_accuracy=True, verbose=2, callbacks=[earlystopper, checkpointer])
...
######################################################################
	...
	print("Quan Saved model to disk")

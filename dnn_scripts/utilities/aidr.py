...
#quan sua seed=113 ->seed =7
def load_and_numberize_data(path="../data/", nb_words=None, maxlen=None, seed=113, start_char=1, oov_char=2, index_from=3, init_type="random", embfile=None, validate_train_merge=0, map_labels_to_five_class=0):
...
#        print "Reading vocabulary from" + filename
	...
	print (filename)
        for rowid, row in enumerate (reader):
            ...
            elif re.search("validate.csv", filename.lower()):    
                sentences_validate.append(row[1])    
                y_validate.append(row[2])    
    ...
    print "Nb of tweets: train: " + str (len(sentences_train)) + " test: " + str (len(sentences_test)) + " validate: " + str (len(sentences_validate))
    ...
    X_validate   = numberize_sentences(sentences_validate,   vocab_idmap, oov_char=oov_char)
    ...
    if map_labels_to_five_class:                
        ...
        y_validate   = remap_labels(y_validate,   merge_labels=merge_labels)

    label_list = sorted( list (set(y_train)))    #label id is sorted
    label_map  = {}
    ...
    (y_validate,   y_validate_freq)     = numberize_labels(y_validate,   label_map)
    #print y_train #Quan them vao
    #print y_train_freq #Quan them vao


    assert len(X_train) == len(y_train) or len(X_test) == len(y_test) or len(X_validate) == len(y_validate)
    ...
    X_validate,   y_validate   = adjust_index(X_validate,   y_validate,   start_char=start_char, index_from=index_from, maxlen=maxlen)
    X_newinput,  y_newinput  = adjust_index(X_newinput,  y_newinput,  start_char=start_char, index_from=index_from, maxlen=maxlen)


    #print X_train    #quan them vao
    #print y_train  #quan them vao

    if validate_train_merge:
        X_train.extend(X_validate)
        y_train.extend(y_validate)
#       y_train=np.concatenate ((y_train, y_validate)) # need if y_train is numpy array
    ...
    return (X_train, y_train), (X_test, y_test), (X_validate, y_validate), (X_newinput,y_newinput), nb_words + index_from, E, label_map
########################################
##Quan sua lai cho fan loadAndUsingmodel
########################################
def load_and_numberize_data2(path="../data/", nb_words=None, maxlen=None, seed=113, start_char=1, oov_char=2, index_from=3, init_type="random", embfile=None, validate_train_merge=0, map_labels_to_five_class=0):
